...
def load_medicare_spending():
    ...
    for yr in years:
        ...
        df['state'] = [name[:2] if isinstance(name, str) else 'unknown' for name in df['county_name']]
        df['county_excl_state'] = [name[3:] if isinstance(name, str) else 'unknown' for name in df['county_name']]
        df_list.append(df)
...
def load_pc_access():
    ...
    '''
    years = ['0307','2008','2009','2010','2011','2012','2013','2014']
    df_list = []
    for yr in years:
